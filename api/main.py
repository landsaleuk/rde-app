...
def build_where_parcels(params, values):
    ...
    # boolean excludes
    if params.get("exclude_offshore"):       where.append("NOT is_offshore")
    if params.get("exclude_road_corridor"):  where.append("NOT is_road_corridor")
    if params.get("exclude_rail_corridor"):  where.append("NOT is_rail_corridor")
    if params.get("exclude_roadlike"):       where.append("NOT is_roadlike_longthin")
...
def list_parcels(
    ...
    allowed_sorts = {
        "parcel_id","cohort","acres","uprn_count","water_pct","land_pct"
    }
    ...
    sql_page = f"""
      SELECT parcel_id, cohort, acres, uprn_count,
             water_pct, land_pct,
    ...
    return {"page": page, "page_size": page_size, "total": total,
            "pages": math.ceil(total / page_size) if page_size else 1, "items": items}
    ...
    def stream_select():
        ...
        with get_conn() as conn:
            with conn.cursor(name="csv_cur") as cur:
                cur.itersize = chunk
                cur.execute(f"""
                  SELECT parcel_id, cohort, acres, uprn_count AS uprn_total,
                         water_pct, land_pct,
                         is_road_corridor, is_rail_corridor, is_offshore
                  FROM parcel_catalog
                  {where_sql}
                ...
                for rec in cur:
                    row = [
                        rec["parcel_id"], rec["cohort"], rec["acres"], rec["uprn_total"],
...
def parcel_uprns(parcel_id: int):
    """
    Return all UPRNs mapped to this parcel (no interior flag), with WGS84 coords.
    """
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
...
def list_uprns(
    ...
    if sort_by not in allowed_sorts: sort_by = "uprn"
    sort_dir = "desc" if sort_dir.lower()=="desc" else "asc"
...
def uprns_csv(
    ...
    params = {"min_acres": min_acres, "max_acres": max_acres,
              "min_water_pct": min_water_pct, "max_water_pct": max_water_pct,
              "min_land_pct": min_land_pct, "max_land_pct": max_land_pct}
    values = []
    where_sql = build_where_uprn(params, values)
