...
def get_conn():
    # row_factory=dict_row makes cursor fetches return dicts
...
def parcels_csv(
    ...
    def stream():
        # Use a server-side cursor in psycopg3: name=... streams rows
        with get_conn() as conn, conn.cursor(name="csv_cur") as cur:
            cur.execute(f"""
                SELECT parcel_id, cohort, acres,
                       uprn_count AS uprn_total,
                       uprn_interior_count AS uprn_interior,
...
def parcel_uprns(parcel_id: int):
    with get_conn() as conn, conn.cursor() as cur:
        # First try precomputed interior (parcel_interior); if missing, compute on the fly
        try:
            cur.execute("""
                SELECT b.uprn,
