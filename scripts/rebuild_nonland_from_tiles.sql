...
-- Flags (NO water). Parcels made valid; land via **tiles** join (fast, indexed).
...
land_area AS (
  -- Sum intersections with **non-overlapping** tiles (ST_Subdivide pieces don’t overlap)
  SELECT b.parcel_id,
...
land_centroid AS (
  -- Fast offshore test: does the parcel centroid fall inside ANY tile?
  SELECT b.parcel_id,
         CASE
           WHEN EXISTS (
...
SELECT
  ...
  (la.a_land / NULLIF(b.a_parcel,0))::numeric(6,4) AS land_ratio,
  ...
  (4*PI()*b.a_parcel / NULLIF( ST_Perimeter(b.geom)::double precision * ST_Perimeter(b.geom)::double precision, 0))::numeric(6,4)
    AS compactness,
  CASE
    WHEN lc.offshore_centroid OR (la.a_land / NULLIF(b.a_parcel,0)) < 0.05 THEN TRUE
  ...
  END AS is_nonland,
  CASE
    WHEN lc.offshore_centroid OR (la.a_land / NULLIF(b.a_parcel,0)) < 0.05 THEN 'offshore/land<5%'
...
FROM base b
LEFT JOIN land_area    la USING (parcel_id)
LEFT JOIN land_centroid lc USING (parcel_id)
LEFT JOIN r             USING (parcel_id)
LEFT JOIN rl            USING (parcel_id);

CREATE INDEX parcel_nonland_flags_pid_ix ON parcel_nonland_flags (parcel_id);
ANALYZE parcel_nonland_flags;

-- Cohorts (unchanged rules), **temporarily** without nfl_gate (we'll add gate next)
...
-- Map (no hard gate yet; we'll swap to nfl_gate next)
